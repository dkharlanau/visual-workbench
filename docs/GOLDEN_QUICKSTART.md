...
node dist/cli.js render \
  ...
  -o /tmp/visual-workbench-v0.1.0.svg
```

Verify the released Node.js 24 rendering digest:

```bash
node -e 'const fs=require("node:fs"); const c=require("node:crypto"); const p="/tmp/visual-workbench-v0.1.0.svg"; const actual=c.createHash("sha256").update(fs.readFileSync(p)).digest("hex"); const expected="4d3ab1393e76b7ea2a8f1462634635eeb46f2e07c135b1e9200e8eb8fceace2e"; if(actual!==expected) throw new Error(`${actual} != ${expected}`); console.log(`verified ${actual}`)'
```
